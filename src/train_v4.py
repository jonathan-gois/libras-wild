"""
Treinamento v4: Transformer SEM mirror augmentation.

Diferença em relação ao v3:
  - mirror_aug = False  (investigação do impacto do espelhamento)
  - n_extra_aug = 2     (compensa a redução de dados sem mirror)

Motivação:
  Em Libras, os sinais têm lateralidade linguística definida. Espelhar o sinal
  cria uma configuração geometricamente válida mas linguisticamente inválida:
    * Sinais unimanuais ficam executados com a mão não-dominante
    * O espelho pode coincidir com outro sinal existente (par mínimo)
    * Introduz ruído semântico no conjunto de treino

Uso:
    cd /home/jonathangois/Documentos/Libras_2026
    source env/bin/activate
    python3 -u src/train_v4.py
"""
...
CFG = dict(
    ...
    patience       = 20,
    seed           = 42,
    mirror_aug     = False,   # DESLIGADO — ver motivação no docstring
    n_extra_aug    = 2,       # 2 versões extras para compensar
